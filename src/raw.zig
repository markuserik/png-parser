...
pub fn parseRaw(raw_file: []u8, input_allocator: std.mem.Allocator) !Png {
    ...
    const signature: u64 = try reader.takeInt(u64, endian);
    ...
    while (true) {
        ...
        switch (chunk.type) {
            .IHDR => {
                ...
                png.ihdr = try IHDR.parse(chunk, endian);
            },
            .PLTE => {
            ...
            .cHRM => {
                ...
                png.chrm = try cHRM.parse(chunk, endian);
            },
            .gAMA => {
            ...
            .bKGD => {
                ...
                png.bkgd = try bKGD.parse(chunk, png.ihdr.?.color_type, endian);
            },
            .tIME => {
                if (png.time != null) return ChunkOrderingError.MultipletIME;
                png.time = try tIME.parse(chunk, endian);
            },
            .tEXt => {
